...
public final class Utils
{
	...
	public static void FillWithGradient(Mat background, Point inters, Point leftBottomStripe, Point rightBottomStripe){
		double two3ds = Math.ceil((inters.y-leftBottomStripe.y)*2/3);	//parte che verrà colorata
		Point[] twoThirds = new Point[2];
		twoThirds[0] = new Point(0, two3ds);
		twoThirds[1] = new Point(Double.MAX_VALUE, two3ds);
		MatOfPoint bottom = new MatOfPoint(Intersection(twoThirds[0], twoThirds[1], inters, leftBottomStripe), leftBottomStripe, Intersection(twoThirds[0], twoThirds[1], inters, rightBottomStripe), rightBottomStripe);
		Imgproc.fillConvexPoly(background, bottom, new Scalar(0, 255, 0, 35));
		//int bottom = (int) Math.ceil(twoThirds*1/3);	//avrà l'alpha più scuro in assoluto
